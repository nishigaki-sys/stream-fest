...
import { ListTodo, Plus, Filter, ExternalLink, CornerDownRight } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      ...
      <div className="relative">
        ...
        {/* 【固定：タイトル & 追加ボタン】
            背景を白（不透明）に設定し、下の文字が絶対に見えないようにします
        */}
          ...
          <table className="w-full text-left border-collapse table-fixed">
            <thead>
              <tr className="border-b">
                {/* 修正ポイント: 
                  - thに直接 sticky top-[オフセット] を指定
                  - bg-white (または bg-gray-50) を指定して背後を隠す
                  - z-[90] でデータ行(z-index未指定なら0)より確実に上へ
                */}
              ...
              {displayTasks.length > 0 ? displayTasks.map(t => {
                ...
                return (
                  <tr key={t.id} className={`transition-colors cursor-pointer group ${rowBgClass}`} onClick={() => onTaskEdit(t)}>
                    ...
                    <td className={`px-6 py-2 font-bold ${isChild ? 'pl-12 text-gray-500 text-[12px]' : 'text-gray-700 text-sm'}`}>
